...
- ESLint flat config at root ([eslint.config.ts](eslint.config.ts)) — `js.configs.recommended` + `tseslint.configs.recommended`. The `mock_ui/`, `docs/`, `.cursor/`, `.Codex/` paths are ignored.
